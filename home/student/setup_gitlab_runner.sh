...
docker run --rm -v $(pwd)/config:/etc/gitlab-runner gitlab/gitlab-runner:latest register --non-interactive \
  ...
  --tls-ca-file="/home/student/ca.local.crt"

# === 5.1 Ustawienie tls_skip_verify w config.toml ===
echo "[5.1] Konfiguracja tls_skip_verify..."
CONFIG_FILE="/etc/gitlab-runner/config.toml"
if grep -q "tls_skip_verify" "$CONFIG_FILE"; then
    sed -i 's/tls_skip_verify = .*/tls_skip_verify = true/' "$CONFIG_FILE"
else
    sed -i "/^\[\[runners\]\]/a tls_skip_verify = true" "$CONFIG_FILE"
fi
